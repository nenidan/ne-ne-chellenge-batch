...
public class CalculateRewardProcessor implements ItemProcessor<ChallengeResult, Reward> {
    ...
    private List<Long> getWinners(Long challengeId, Integer totalDays) {
        Map<Long, Integer> userHistoryCount = getUserHistoryCount(challengeId);

        List<Long> winners = userHistoryCount.entrySet().stream()
    ...
    private Map<Long, Integer> getUserHistoryCount(Long challengeId) {
        String sql = """
